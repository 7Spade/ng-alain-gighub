...
> **目的**：展示帳戶層架構，包含認證系統、帳戶類型、組織協作等核心功能

**最後更新**：2025-11-15  
**版本**：v2.0（Git-like 分支模型 + 認證系統整合）  
**實現狀態**：
- ✅ **認證系統已實現**：Supabase Auth + @delon/auth 協作整合
- ✅ **帳戶資料模型**：Account 統一身份抽象（User/Bot/Organization）
- ✅ **Session 適配器**：SupabaseSessionAdapter 負責認證系統橋接
...
- **AuthService** (`src/app/shared/services/auth/auth.service.ts`)
  - 提供 signIn, signUp, signOut, getCurrentUser 等業務方法
  - 整合 Supabase Auth 與 AccountRepository
...
- **AccountRepository** (`src/app/core/repositories/account.repository.ts`)
  - 訪問 accounts 資料表
  - 依 auth_user_id 查詢完整帳戶資料
...
flowchart TD
    %% ==================== 認證系統層 (已實現) ====================
    subgraph AuthLayer["🔐 認證系統層 (Supabase + Delon Auth)"]
        direction TB
        
        subgraph SupabaseAuth["Supabase Auth<br/>(底層認證服務)"]
            direction TB
            SupaLogin["signInWithPassword"]
            SupaSignup["signUp"]
            SupaSession["Session 管理"]
            SupaJWT["JWT Token"]
            
            SupaLogin --> SupaSession
            SupaSignup --> SupaSession
            SupaSession --> SupaJWT
        end
        
        subgraph Adapter["SupabaseSessionAdapter<br/>(Session 轉換層)"]
            direction TB
        ...
        subgraph DelonAuth["@delon/auth<br/>(前端認證框架)"]
            direction TB
            TokenService["TokenService<br/>(Token 儲存)"]
            AuthGuard["authSimpleCanActivate<br/>(路由守衛)"]
            AuthInterceptor["authSimpleInterceptor<br/>(HTTP 攔截器)"]
            
            TokenService --> AuthGuard
            TokenService --> AuthInterceptor
        end
        
        subgraph AuthBusiness["AuthService<br/>(業務層)"]
            direction TB
            SignIn["signIn()<br/>(登入)"]
            SignUp["signUp()<br/>(註冊)"]
            SignOut["signOut()<br/>(登出)"]
            GetUser["getCurrentUser()<br/>(獲取用戶)"]
        end
    ...
    %% 認證系統到帳戶層的連接
    AuthBusiness -.認證成功後載入.-> Account
    SupaJWT -.auth_user_id 關聯.-> Account
    ...
    subgraph PermissionLayer["🔒 權限管理 (RLS + Branch Roles)"]
        ...
        BranchRoleNode --> BranchPermissionNode --> RLSPolicies
    end
    
    AccountLayer -.JWT Claims.-> PermissionLayer
    PermissionLayer -.授權.-> BlueprintMgmt
    PermissionLayer -.授權.-> Execution
    PermissionLayer -.授權.-> Collaboration
    ...
    class SupaLogin,SupaSignup,SupaSession,SupaJWT supabaseStyle
