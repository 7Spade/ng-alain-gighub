...
> **日期**：2025-01-20  
> **目的**：设计现代化的隐私权控制方案，支持用户、组织、蓝图的搜索和隐私保护  
> **状态**：📋 设计方案
...
1. **可搜索性**：用户、组织、蓝图都应该能被搜索到
2. **隐私保护**：不同实体可以设置不同的隐私级别
3. **细粒度控制**：支持字段级别的隐私控制
4. **关系感知**：基于用户关系（好友、组织成员、协作关系）的权限控制
5. **现代化设计**：符合业界最佳实践，易于扩展和维护
...
参考业界最佳实践（GitHub、LinkedIn、Twitter 等），采用**可见性级别 + 关系感知**的混合模式：
...
CREATE OR REPLACE FUNCTION public.search_accounts_for_explore(
  ...
  p_viewer_auth_user_id UUID DEFAULT NULL  -- 新增：查看者 ID
)
RETURNS TABLE (...)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT 
    a.id,
    a.name,
    -- 根据 contact_visibility 决定是否返回邮箱
    CASE 
...
```typescript
// 搜索结果组件中，根据可见性显示/隐藏字段
@if (canViewContact(account)) {
  <div class="email">{{ account.email }}</div>
} @else {
  <div class="email-placeholder">联系信息不可见</div>
}
```
...
1. ✅ 添加 `search_visibility` 字段到 `accounts` 表
2. ✅ 添加 `search_visibility` 字段到 `blueprints` 表
3. ✅ 创建可见性级别枚举类型
4. ✅ 设置默认值（`authenticated`）

### 阶段 2：权限判断函数（2-3 天）

1. ✅ 创建 `can_view_account` 函数
2. ✅ 创建 `can_view_blueprint` 函数
3. ✅ 测试权限判断逻辑

### 阶段 3：搜索函数更新（1-2 天）

1. ✅ 更新 `search_accounts_for_explore` 函数
2. ✅ 更新 `search_blueprints_for_explore` 函数（如果需要）
3. ✅ 更新 Repository 调用，传递 `viewer_auth_user_id`
...
- **用户账户**：`search_visibility = 'authenticated'`（默认已登录用户可见）
- **组织账户**：`search_visibility = 'authenticated'`（默认已登录用户可见）
- **蓝图**：`search_visibility = 'organization'`（默认组织成员可见）
...
1. **好友功能**：是否需要实现好友功能？如果不需要，可以移除 `friends` 可见性级别
2. **默认隐私级别**：各实体的默认隐私级别是否合适？
3. **字段可见性**：是否需要字段级别的可见性控制，还是实体级别的就够了？
4. **组织可见性**：组织账户的 `organization` 可见性是指"同一组织的成员"还是"所有组织成员"？
