...
清理 `src/app/shared/models` 目錄中的遺留文件，解決文件命名不一致問題，確保與 `src/app/core/infra/types` 無衝突。

### 問題分析

1. **文件命名不一致**：模組化目錄使用 `types.ts`，但存在兩個遺留文件使用 `.model.ts` 後綴
2. **內容重複定義**：`activity-log.model.ts` 和 `quality-check.model.ts` 的內容已遷移到模組目錄
3. **潛在混淆**：新開發者可能不知道應該使用哪個文件
...
1. **問題分析**
   - ✅ 檢查遺留文件使用情況（確認無直接導入）
   - ✅ 檢查與 `core/infra/types` 的衝突（確認無衝突）
   - ✅ 創建詳細分析報告

2. **文件清理**
   - ✅ 刪除 `activity-log.model.ts`
   - ✅ 刪除 `quality-check.model.ts`
   - ✅ 更新 `index.ts` 註釋
...
## 2025-01-15（下午）: Bot 與組織功能完善

### 背景

完善 Bot 賬戶系統，實現個人 Bot 與組織 Bot 的區分，並將團隊和排班功能改為組織專有功能。

### 決策與權衡

#### 1. Bot 類型區分方案

**問題**：如何區分個人 Bot 和組織 Bot？

**方案選擇**：
- ✅ **方案 A**：使用 `auth_organization_id` 字段區分（NULL = 個人 Bot，有值 = 組織 Bot）
- ❌ **方案 B**：添加新字段 `owner_account_id`
- ❌ **方案 C**：使用 `metadata` JSONB 字段

**決策**：選擇方案 A，因為：
1. 不需要修改表結構，復用現有字段
2. 語義清晰：`auth_bot_id` 記錄創建者，`auth_organization_id` 記錄所屬組織
3. 與 Organization 賬戶的設計一致

#### 2. 團隊和排班組織專有化

**問題**：如何確保團隊和排班功能只顯示用戶所屬組織的數據？

**方案選擇**：
- ✅ **方案 A**：添加組織選擇器，用戶必須先選擇組織
- ❌ **方案 B**：自動過濾，不顯示選擇器

**決策**：選擇方案 A，因為：
1. 明確數據範圍：用戶可能屬於多個組織
2. 符合業務邏輯：團隊和排班是組織專有功能
3. 提升用戶體驗：自動選擇單個組織，減少操作步驟
...
1. **Bot 區分機制**
   - ✅ 更新 `create_bot_account` 函數，支持 `p_organization_id` 參數
   - ✅ 更新 RLS 策略，支持組織成員查看組織 Bot
   - ✅ 清理舊數據（`auth_bot_id` 為 NULL 的 Bot）
   - ✅ 前端添加 `personalBotAccounts` 和 `organizationBotAccounts` computed signals

2. **團隊和排班組織專有化**
   - ✅ TeamListComponent 添加組織選擇器
   - ✅ ScheduleListComponent 添加組織選擇器
   - ✅ 使用 `loadTeamsByOrganizationId` 和 `loadSchedulesByOrganizationId`
   - ✅ 改進用戶體驗（自動選擇、友好提示）
...
1. **類型定義更新**：使用 Supabase MCP 工具重新生成類型定義，移除臨時類型斷言
2. **CreateBotComponent 擴展**：添加組織選擇器，支持創建組織 Bot
3. **性能優化**：優化 bundle 大小（當前 3.49 MB > 2.00 MB 預算）

### 相關文檔

- [工作總結-2025-01-15-Bot與組織功能完善.md](./工作總結-2025-01-15-Bot與組織功能完善.md)
...
### 背景
執行完整的開發工作流程，從專案脈絡回顧、構建檢查、修復問題到自驗收測試，確保專案處於可交付狀態。

### 問題與解決

#### ESLint 配置問題
- **問題**：285 個 ESLint 解析錯誤，錯誤信息 `Enabling "project" does nothing when "projectService" is enabled`
- **原因**：`eslint.config.mjs` 中同時設置了 `projectService: true` 和 `project: ['tsconfig.json']`
- **解決方案**：移除 `project: ['tsconfig.json']` 設置，保留 `projectService: true`
- **結果**：✅ 285 個錯誤全部解決
...
#### 發現的問題
1. **Bundle 大小警告**：超出預算 1.49 MB（性能優化問題）
2. **權限同步警告**：`Failed to sync roles: column roles_1.code does not exist`
3. **API 調用錯誤**：400 狀態碼（需檢查 Supabase 配置）
4. **未使用的變數**：部分文件中有未使用的導入和變數
...
- ✅ 開發服務器成功啟動（端口 4200）
- ✅ 登錄頁面正常載入
- ✅ 登錄功能正常（帳號：`ac7x@pm.me`，密碼：`123123`）
- ✅ 儀表板頁面正常顯示
- ✅ 主要功能正常運作

#### 使用的工具
- **Playwright MCP**：自動化測試
- **Sequential Thinking**：問題分析與拆解
- **Software Planning Tool**：任務規劃
...
#### ⏳ 待處理
1. **修復權限同步問題**：檢查 Supabase 資料庫結構，確認 `roles` 表欄位定義
2. **修復 API 調用錯誤**：檢查 Supabase 配置和 API 端點
3. **優化 Bundle 大小**：實施代碼分割和懶加載優化
4. **清理未使用的變數**：使用 ESLint 自動修復或手動清理

#### 📈 後續優化
- 增加類型檢查步驟（建議添加 `type-check` 腳本）
- 完善 CI/CD 流程（自動化構建、測試、部署）
- 提高測試覆蓋率（為藍圖系統、帳戶系統添加測試）

### 參考文檔
- [完整工作總結](./工作總結-完整開發工作流程執行-2025-01-15.md)
