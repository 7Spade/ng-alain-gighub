...
由於 `account-detail.component.ts` 的模板中有以下條件判斷：
...
1. 用戶點擊「查看」按鈕
2. 導航到 `/accounts/:id`
3. `AccountDetailComponent.ngOnInit()` 執行
4. `loadAccountById()` 完成，設置 `AccountService.loading = false`
5. **但同時** `OrganizationRoleManageComponent.ngOnInit()` 執行
6. `loadAccounts()` 設置 `AccountService.loading = true`
7. 頁面條件判斷 `accountService.loading()` 為 `true`
8. **結果**：頁面一直顯示加載狀態

### 為什麼會這樣？
- `AccountService` 是單例服務（`providedIn: 'root'`）
- `loading` 狀態是全局共享的
- 子組件（`OrganizationRoleManageComponent`）的操作會影響父組件（`AccountDetailComponent`）的顯示
- 這是典型的**狀態管理不當**問題
...
### 修復策略
1. **避免阻塞主流程**：不等待 `loadAccounts()` 完成
2. **按需加載**：只在需要時才加載賬戶列表
3. **非阻塞加載**：使用 `.catch()` 而不是 `await`，避免阻塞渲染
...
3. **非阻塞**：使用 `.catch()` 而不是 `await`，不阻塞頁面渲染
...
- [架構文檔](./fyi-architecture.md#賬戶系統架構)
