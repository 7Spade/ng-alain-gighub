...
Shared component 必須具備 inputs / outputs 明確定義，不可綁定特定 domain。

Component 必須「單一責任」：UI 相關 → UI；邏輯 → Facade；資料 → Repository。

Shared component 禁止耦合任何 domain model 或 service。
...
多次使用的 business logic 必須抽到 Domain Service，禁止藏在 component。
