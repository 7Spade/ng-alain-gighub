# 工作總結 - accounts 表 RLS 遞歸問題修復完整流程

> **日期**：2025-01-15  
> **狀態**：✅ 完成並驗證通過

---

## 📋 任務概述

修復 accounts 表的 RLS（Row Level Security）遞歸問題，該問題導致查詢 accounts 表時返回 500 錯誤：`"infinite recursion detected in policy for relation \"accounts\""`。

---

## 🔍 問題診斷

### 問題發現

1. **初始問題**：用戶註冊後沒有自動創建 account 記錄
   - **解決方案**：創建了 `handle_new_user()` 觸發器自動創建 account

2. **後續問題**：accounts 表查詢返回 500 錯誤
   - **錯誤信息**：`"infinite recursion detected in policy for relation \"accounts\""`
   - **根本原因**：RLS 策略中直接查詢 accounts 表，形成循環查詢

### 問題根源

accounts 表的 SELECT 策略中存在遞歸查詢：

```sql
-- 原始策略（有問題）
tm.account_id = (
  SELECT accounts_1.id
  FROM accounts accounts_1  -- ⚠️ 觸發 RLS 檢查，形成遞歸
  WHERE accounts_1.auth_user_id = auth.uid()
)
```

**遞歸鏈**：
- 查詢 `accounts` 表 → 觸發 RLS 策略檢查
- 策略中又查詢 `accounts` 表 → 再次觸發 RLS 策略檢查
- 無限循環 → 遞歸錯誤

---

## 🔧 解決方案

### 參考 Supabase 官方文檔

查閱 Supabase 官方文檔後，確認使用 **SECURITY DEFINER 函數**是官方推薦的解決方案。

**參考文檔**：
- [Row Level Security 文檔](https://supabase.com/docs/guides/database/postgres/row-level-security#use-security-definer-functions)
- 在 "Use security definer functions" 章節中有詳細說明

### 實施步驟

#### 1. 創建 private schema

```sql
create schema if not exists private;
```

#### 2. 創建 SECURITY DEFINER 函數

**`private.is_user_org_member`**：
- 功能：檢查用戶是否是組織成員
- 參數：`org_account_id UUID`, `user_auth_id UUID`
- 返回：`boolean`
- 安全級別：`SECURITY DEFINER`

**`private.is_user_org_admin`**：
- 功能：檢查用戶是否是組織管理員
- 參數：`org_account_id UUID`, `user_auth_id UUID`
- 返回：`boolean`
- 安全級別：`SECURITY DEFINER`

#### 3. 更新 RLS 策略

- 刪除舊的 SELECT 和 UPDATE 策略
- 創建新策略使用 SECURITY DEFINER 函數

---

## ✅ 修復結果

### 遷移文件

**遷移名稱**：`fix_accounts_rls_recursion_with_security_definer`

**包含內容**：
1. ✅ 創建 `private` schema
2. ✅ 創建 `private.is_user_org_member()` 函數
3. ✅ 創建 `private.is_user_org_admin()` 函數
4. ✅ 更新 accounts 表的 SELECT 策略
5. ✅ 更新 accounts 表的 UPDATE 策略

### 驗證結果

#### 修復前

- ❌ `GET /rest/v1/accounts?select=*` → 500 Internal Server Error
- ❌ 錯誤：`"infinite recursion detected in policy for relation \"accounts\""`
- ❌ 頁面無法顯示數據

#### 修復後

- ✅ `GET /rest/v1/accounts?select=*` → 200 OK
- ✅ 成功返回數據：
```json
[{
  "id": "cdfc428d-d23d-4a7d-a881-9f10e921d85f",
  "auth_user_id": "037e1c67-3976-4c55-ba3a-e32982d7c9ef",
  "type": "User",
  "name": "ac7x",
  "email": "ac7x@pm.me",
  "status": "active",
  ...
}]
```
- ✅ 頁面正常顯示數據
- ✅ 響應時間：4ms（性能良好）

### 功能驗證

使用 MCP 工具登入驗證（帳號：ac7x@pm.me，密碼：123123）：

1. ✅ **登入成功**：POST /auth/v1/token → 200 OK
2. ✅ **查詢成功**：GET /rest/v1/accounts → 200 OK
3. ✅ **頁面顯示**：成功顯示 account 列表
4. ✅ **數據正確**：顯示用戶的 account 記錄

---

## 🔑 技術要點

### SECURITY DEFINER 函數的作用

1. **繞過 RLS 檢查**：
   - 函數以創建者（`postgres` 角色）的權限執行
   - 創建者具有 `bypassrls` 權限
   - 函數內部查詢不受 RLS 限制

2. **避免遞歸**：
   - 函數內部查詢 `accounts` 表時不會觸發 RLS 策略
   - 打破循環查詢鏈，避免無限遞歸

3. **安全性**：
   - 使用 `set search_path = ''` 防止 search_path 注入攻擊
   - 函數放在 `private` schema 中，不在 "Exposed schemas" 中

### 修復前後對比

| 項目 | 修復前 | 修復後 |
|------|--------|--------|
| accounts 查詢 | ❌ 500 Error | ✅ 200 OK |
| 錯誤信息 | `"infinite recursion..."` | ✅ 無錯誤 |
| 頁面顯示 | ❌ 顯示錯誤 | ✅ 正常顯示數據 |
| 響應時間 | N/A（失敗） | ✅ 4ms |
| 函數 | ❌ 不存在 | ✅ 2 個 SECURITY DEFINER 函數 |
| 策略 | ❌ 有遞歸問題 | ✅ 使用 SECURITY DEFINER 函數 |

---

## 📚 相關文檔

### 創建的文檔

1. **`docs/Supabase-RLS遞歸問題處理方法.md`**
   - Supabase 官方處理 RLS 遞歸問題的方法
   - 包含完整的實施步驟和示例

2. **`docs/工作總結-修復失敗原因分析-2025-01-15.md`**
   - 分析為什麼第一次修復會失敗（被復原了）
   - 說明問題根源和解決方案

3. **`docs/工作總結-accounts-RLS修復完成-2025-01-15.md`**
   - 詳細的修復過程和驗證結果

4. **`docs/工作總結-最終驗證-accounts-RLS修復-2025-01-15.md`**
   - 使用 MCP 工具的最終驗證結果

### 參考資源

- [Supabase RLS 文檔](https://supabase.com/docs/guides/database/postgres/row-level-security)
- [SECURITY DEFINER 函數說明](https://supabase.com/docs/guides/database/postgres/row-level-security#use-security-definer-functions)
- [RLS 性能最佳實踐](https://github.com/orgs/supabase/discussions/14576)

---

## 🎯 最終結論

**accounts 表 RLS 遞歸問題已完全修復並驗證通過！**

### 成功項目

1. ✅ **修復成功**：使用 Supabase 官方推薦的 SECURITY DEFINER 函數方法
2. ✅ **查詢正常**：accounts 表查詢返回 200 OK
3. ✅ **數據正確**：成功返回用戶的 account 記錄
4. ✅ **頁面正常**：account 列表頁面正常顯示
5. ✅ **性能良好**：查詢響應時間 4ms
6. ✅ **權限控制**：RLS 策略正常工作，權限控制邏輯保持不變

### 經驗總結

1. **問題診斷**：正確識別問題根源（RLS 策略遞歸）
2. **解決方案**：參考官方文檔，使用標準解決方案
3. **驗證方法**：使用 MCP 工具進行端到端驗證
4. **文檔記錄**：完整記錄問題、解決方案和驗證過程

---

**最後更新**：2025-01-15  
**維護者**：開發團隊

