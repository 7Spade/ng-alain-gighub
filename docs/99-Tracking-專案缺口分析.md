...
**建議解決方案**：
1. ✅ 設計任務對應關係機制（已完成，參考 [PR合併邏輯實現方案-2025-01-20.md](./PR合併邏輯實現方案-2025-01-20.md)）
2. ⏳ 實現 TaskService.updateTaskContractorFields() 方法
3. ⏳ 在 Service 層實現 PR 合併邏輯（不使用 Edge Functions，簡化架構）
4. ⏳ 實現完整的錯誤處理和日誌記錄
5. ⏳ 補充集成測試

**設計決策**：不使用 Edge Functions，在 Service 層直接實現 PR 合併邏輯，因為工地任務系統的合併邏輯比 GitHub 簡單，不需要複雜的服務端處理。
...
**問題描述**：
專案缺少自動化的 CI/CD 流程，無法自動化測試、構建和部署。

**待實現功能**：
- GitHub Actions 工作流配置
- 自動化測試流程（單元測試、集成測試）
- 自動化構建流程（生產環境構建）
- 自動化部署流程（Supabase Hosting）
- 環境變數管理（Secrets 管理）
- 版本標籤管理
- 預發布環境部署
...
**預計工作量**：5-7 天
- GitHub Actions 工作流配置：2-3 天
- 自動化測試流程：1-2 天
- 自動化構建和部署：1-2 天
- 環境變數和版本管理：1 天

**建議優先級**：
1. 建立基礎 CI/CD 流程（測試 + 構建）
2. 建立自動化部署流程
3. 建立預發布環境
4. 完善版本管理
...
### 7. 探索功能（Explore）- 功能不完整

**問題描述**：
探索功能允許用戶搜索用戶、組織、藍圖，但隱私權控制功能未實現。

**當前狀態**：
- ✅ 組件已實現（`routes/explore/explore.component.ts`）
- ✅ Service 已實現（`ExploreService`）
- ✅ 基礎搜索功能已實現
- ✅ 隱私權控制方案已設計（參考 [探索功能隐私权控制方案-2025-01-20.md](./探索功能隐私权控制方案-2025-01-20.md)）

**待實現**：
- ⏳ 隱私權控制功能（可見性級別、關係感知權限）
- ⏳ RLS 策略（搜索權限控制）
- ⏳ 搜索結果的隱私過濾
- ⏳ 字段級別隱私控制
- ⏳ 測試覆蓋
...
**問題描述**：
待辦中心組件已實現，但部分核心功能未完成。

**當前狀態**：
- ✅ 組件已實現（`routes/communication/todos/todo-center.component.ts`）
- ✅ Service 已實現（`PersonalTodoService`）
- ✅ 五種狀態分類已實現
- ✅ 篩選功能已實現

**待實現**：
- ⏳ 完成功能（`complete()` 方法有 TODO）
- ⏳ 詳情查看功能（`viewDetail()` 方法有 TODO）
- ⏳ 批量操作功能
- ⏳ 待辦狀態自動同步
- ⏳ 待辦提醒功能
...
**問題描述**：
文檔管理系統的 Facade 和 Repository 已實現，但 UI 頁面功能未實現。

**當前狀態**：
- ✅ DocumentFacade 已實現
- ✅ StorageFacade 已實現
- ✅ DocumentRepository 已實現
- ✅ 9 個頁面組件骨架存在

**待實現的頁面功能**：
- ⏳ 文檔列表頁面功能（DocumentListComponent）
- ⏳ 文檔上傳頁面功能（DocumentUploadComponent）
- ⏳ 文檔瀏覽器功能（DocumentBrowserComponent）
- ⏳ 文檔預覽功能（DocumentPreviewComponent）
- ⏳ 圖紙查看器功能（DrawingViewerComponent）
- ⏳ 文檔元數據管理（DocumentMetadataComponent）
- ⏳ 文檔版本控制 UI（DocumentVersionComponent）
- ⏳ 文檔權限管理 UI（DocumentPermissionComponent）
...
**問題描述**：
Dashboard 是應用的首頁，但目前所有頁面功能未實現。

**待實現的頁面**：
- ⏳ 儀表板分析頁面（AnalysisComponent）
- ⏳ 儀表板監控頁面（MonitorComponent）
- ⏳ 儀表板工作區頁面（WorkplaceComponent）
- ⏳ 儀表板 v1 頁面（V1Component）
...
### 16. Edge Functions - 部分未實現

**設計決策**：PR 合併邏輯（branch-merge）改為在 Service 層實現，不使用 Edge Functions，簡化架構。

**待實現的 Edge Functions**：

| Edge Function | 模組 | 功能 | 狀態 | 備註 |
|--------------|------|------|------|------|
| ~~branch-merge~~ | ~~藍圖系統（M4）~~ | ~~PR 合併承攬欄位~~ | ✅ 改為 Service 層實現 | 參考 [PR合併邏輯實現方案-2025-01-20.md](./PR合併邏輯實現方案-2025-01-20.md) |
| weather-api | 任務執行系統（M5） | 天氣 API 查詢 | ⏳ 待開始 | 可考慮改為 Service 層實現 |
| notification-handler | 協作溝通系統（M8） | 通知處理 | ⏳ 待開始 | 可考慮改為 Service 層實現 |
| progress-calculator | 資料分析系統（M8） | 進度計算 | ⏳ 待開始 | 可考慮改為 Service 層實現 |
| analytics-processor | 資料分析系統（M8） | 數據分析 | ⏳ 待開始 | 可考慮改為 Service 層實現 |
| report-generator | 資料分析系統（M8） | 報表生成 | ⏳ 待開始 | 可能需要 Edge Function（PDF/Excel 生成） |

**預計工作量**：8-12 天（減少 2-3 天，因為 branch-merge 改為 Service 層實現）
- 每個 Edge Function 約 2-3 天（設計 + 實施 + 測試）

**建議優先級**：
1. ~~branch-merge~~（✅ 已改為 Service 層實現）
2. notification-handler（通知系統核心，可考慮改為 Service 層實現）
3. weather-api（施工日誌功能，可考慮改為 Service 層實現）
4. progress-calculator（進度追蹤，可考慮改為 Service 層實現）
5. analytics-processor（數據分析，可考慮改為 Service 層實現）
6. report-generator（報表生成，可能需要 Edge Function）

**備註**：大部分功能可以考慮在 Service 層實現，只有需要複雜服務端處理的功能（如 PDF/Excel 生成）才需要 Edge Functions。
...
**問題描述**：
專案中有多個輔助路由模組，目前功能未實現。

**待實現的模組**：

| 模組 | 頁面數量 | 狀態 | 預計工作量 |
|------|---------|------|-----------|
| Widgets | 1 個 | ⏳ 待開始 | 1-2 天 |
| Style | 3 個 | ⏳ 待開始 | 2-3 天 |
| Delon | 11 個 | ⏳ 待開始 | 5-7 天 |
| Extras | 3 個 | ⏳ 待開始 | 2-3 天 |
| Pro | 5 個 | ⏳ 待開始 | 3-5 天 |
| Exception | 2 個 | ⏳ 待開始 | 1-2 天 |
| Data-V | 1 個 | ⏳ 待開始 | 1-2 天 |

**總計**：26 個頁面，預計工作量 15-24 天
...
**問題描述**：
國際化服務已實現，但翻譯文件完整性和功能完善度待確認。

**當前狀態**：
- ✅ I18NService 已實現
- ✅ 支援 3 種語言（zh-CN, zh-TW, en-US）
- ✅ 基礎 i18n 配置完成
...
**問題描述**：
專案有移動端適配指南，但實際適配工作未開始。

**當前狀態**：
- ✅ 移動端適配指南文檔存在（`docs/55-移動端適配指南.md`）
- ⏳ 響應式設計未完善
- ⏳ 觸控互動未優化
- ⏳ PWA 未實作

**待實現**：
- ⏳ 響應式設計完善（Mobile First）
- ⏳ 觸控互動優化
- ⏳ PWA 實作（Service Worker、Manifest）
- ⏳ 移動端性能優化
- ⏳ 移動端測試
...
| 優先級 | 類別數量 | 總工作量 | 關鍵性 |
|--------|---------|---------|--------|
| 🔴 關鍵（Critical） | 6 項 | 70-99 天 | 阻塞核心功能 |
| 🟡 重要（High） | 7 項 | 60-89 天 | 影響核心功能 |
| 🟢 功能（Medium） | 6 項 | 87-119 天 | 功能完整性 |
| 📚 完善（Low） | 4 項 | 50-69 天 | 完善性 |

**總計工作量**：267-376 天（約 8.9-12.5 個月）
...
2. **PR 合併邏輯完善**（5-7 天）
   ...
   - Service 層實現（不使用 Edge Functions）
...
6. **Supabase Storage 整合**（7-10 天）
   - StorageFacade 實施
   - 照片上傳功能
   - Storage RLS 策略

7. **探索功能和待辦中心完善**（8-12 天）
   - 探索功能隱私權控制（5-7 天）
   - 待辦中心功能完善（3-5 天）

8. **文檔管理 UI 功能**（10-15 天）
   - 文檔列表和上傳
   - 文檔預覽和瀏覽
   - 圖紙查看器
   - 版本控制和權限管理
...
📊 **工作量估算**：
- 總計：267-376 天（約 8.9-12.5 個月）
- 關鍵缺口：70-99 天（約 2.3-3.3 個月）
- 重要缺口：60-89 天（約 2-3 個月）
- 功能缺口：87-119 天（約 2.9-4 個月）
- 完善缺口：50-69 天（約 1.7-2.3 個月）

**更新日期**：2025-01-20  
**更新內容**：補充遺漏的缺口（CI/CD、部署、探索功能、待辦中心、文檔管理、Dashboard、國際化、移動端適配等）
