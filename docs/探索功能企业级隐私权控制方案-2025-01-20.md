...
> **日期**：2025-01-20  
> **目的**：设计符合企业级标准的隐私权控制方案，支持合规性、审计、数据分类和细粒度访问控制  
> **状态**：📋 企业级设计方案  
> **标准参考**：ISO 27001, GDPR, CCPA, NIST Privacy Framework
...
- ✅ **GDPR 合规**：数据最小化、用户同意、数据可携带性、被遗忘权
- ✅ **CCPA 合规**：数据透明度、用户权利、数据删除
- ✅ **ISO 27001**：信息安全管理体系
- ✅ **SOC 2**：安全、可用性、处理完整性、保密性、隐私性
...
└─────────────────────────────────────────────────────────┘
           ↓
┌─────────────────────────────────────────────────────────┐
│  第三层：字段级别控制 (Field-level Control)              │
│  - 基本信息可见性                                        │
│  - 联系信息可见性                                        │
│  - 详细信息可见性                                        │
│  - 元数据可见性                                          │
...
/**
 * 数据分类级别（企业级标准）
 * 参考：ISO 27001, NIST Privacy Framework
 */
...
ALTER TABLE accounts ADD COLUMN IF NOT EXISTS 
  ...
  privacy_consent_version VARCHAR(20),  -- 隐私政策版本号
  
  -- 数据保留策略
  data_retention_policy VARCHAR(50) DEFAULT 'standard',  -- standard, extended, permanent
  data_retention_until TIMESTAMPTZ;  -- 数据保留到期时间
...
CREATE TABLE IF NOT EXISTS privacy_access_logs (
  ...
  ip_address INET,
  user_agent TEXT,
  request_id UUID,  -- 用于追踪完整请求链路
  created_at TIMESTAMPTZ DEFAULT NOW(),
  
  -- 合规性字段
  gdpr_legal_basis VARCHAR(50),  -- GDPR 法律依据
...
-- 分区表（按月分区，提高查询性能）
-- 注意：PostgreSQL 12+ 支持自动分区
```

### 4. 数据分类策略表

```sql
-- 数据分类策略表（定义不同类型数据的默认分类）
...
('account', 'User', 'internal', ARRAY['public', 'authenticated', 'organization', 'collaborator', 'private'], '用户账户默认分类：内部数据'),
('account', 'Organization', 'internal', ARRAY['public', 'authenticated', 'organization', 'collaborator', 'private'], '组织账户默认分类：内部数据'),
('account', 'Bot', 'confidential', ARRAY['organization', 'collaborator', 'private'], 'Bot 账户默认分类：机密数据'),
('blueprint', NULL, 'internal', ARRAY['public', 'authenticated', 'organization', 'collaborator', 'private'], '蓝图默认分类：内部数据');
```
...
BEGIN
  ...
  -- 获取 IP 地址和 User Agent（从当前会话）
  -- 注意：这需要从应用层传递，或使用 PostgreSQL 扩展
...
```sql
-- 创建数据分类检查函数（确保用户设置的可见性级别符合数据分类策略）
...
CREATE OR REPLACE FUNCTION public.search_accounts_for_explore(
  ...
  p_request_id UUID DEFAULT NULL  -- 新增：请求 ID（用于审计追踪）
)
RETURNS TABLE (
  id UUID,
  name VARCHAR,
...
BEGIN
  ...
  SELECT 
    a.id,
    a.name,
    -- 根据 contact_visibility 和数据分类决定是否返回邮箱
    CASE 
    ...
    a.auth_bot_id,
    -- 根据 profile_visibility 和数据分类决定是否返回头像
    CASE 
      WHEN private.can_view_account_enterprise(
        a.id, 
        p_viewer_auth_user_id, 
      ...
      ELSE NULL
    END as avatar_url,
    -- 根据 detail_visibility 和数据分类决定是否返回元数据
    CASE 
      WHEN private.can_view_account_enterprise(
        a.id, 
        p_viewer_auth_user_id, 
  ...
  WHERE 
    ...
    -- 搜索用户时，限制 auth_user_id IS NOT NULL
    AND (
      p_type IS NULL 
      OR p_type != 'User' 
      OR a.auth_user_id IS NOT NULL
    )
    -- 企业级隐私控制：只返回可见的账户（基于数据分类和可见性级别）
    AND (
      -- 公开数据：所有人可见
...
CREATE OR REPLACE FUNCTION public.delete_user_data(
  p_user_auth_id UUID,
  p_retention_period INTERVAL DEFAULT '30 days'  -- 软删除保留期
)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
...
@Component({
  ...
  template: `
    <nz-card nzTitle="企业级隐私设置">
      <!-- 数据分类显示（只读，由系统管理员设置） -->
      <nz-alert
        nzType="info"
        nzMessage="数据分类"
        [nzDescription]="'当前数据分类：' + dataClassification()"
      ...
      <nz-form [formGroup]="privacyForm">
        <nz-form-item>
          <nz-form-label>搜索可见性</nz-form-label>
          <nz-form-control>
      ...
      <!-- GDPR 合规选项 -->
      <nz-divider nzText="GDPR 合规"></nz-divider>
  ...
  getVisibilityOptions(classification: DataClassification): Array<{value: VisibilityLevel, label: string}> {
    switch (classification) {
      case DataClassification.PUBLIC:
        return [
          { value: VisibilityLevel.PUBLIC, label: '公开 - 所有人可见' },
          { value: VisibilityLevel.AUTHENTICATED, label: '已认证用户' },
          { value: VisibilityLevel.ORGANIZATION, label: '组织成员' },
          { value: VisibilityLevel.COLLABORATOR, label: '协作成员' },
          { value: VisibilityLevel.PRIVATE, label: '私有' }
        ];
      ...
      case DataClassification.RESTRICTED:
        return [
          { value: VisibilityLevel.PRIVATE, label: '私有 - 仅自己可见' }
        ];
      default:
        return [];
    }
...
```typescript
// 搜索结果组件中，显示数据分类标签和脱敏信息
@if (canViewContact(account)) {
  <div class="email">{{ account.email }}</div>
} @else {
  <div class="email-placeholder">
    {{ maskEmail(account.email) }}  <!-- 显示脱敏后的邮箱 -->
  </div>
}
...
### 阶段 1：数据分类和基础字段（3-5 天）

1. ✅ 创建数据分类枚举和策略表
2. ✅ 添加数据分类字段到 `accounts` 和 `blueprints` 表
3. ✅ 添加隐私控制字段
4. ✅ 设置默认数据分类策略
...
| 数据分类 | 默认可见性级别 | 允许的可见性级别 | 适用场景 |
|---------|--------------|----------------|---------|
| **Public** | `authenticated` | `public`, `authenticated`, `organization`, `collaborator`, `private` | 公开信息、营销内容 |
| **Internal** | `organization` | `authenticated`, `organization`, `collaborator`, `private` | 内部文档、团队信息 |
| **Confidential** | `collaborator` | `organization`, `collaborator`, `private` | 敏感项目、客户数据 |
| **Restricted** | `private` | `private` | 个人隐私、财务数据 |
...
- ✅ 所有隐私数据访问都记录到 `privacy_access_logs`
- ✅ 日志保留 1 年（合规要求）
- ✅ 支持按资源类型、数据分类、访问者查询
- ✅ 定期生成隐私访问报告
...
- ✅ GDPR：数据最小化、用户同意、数据可携带性、被遗忘权
- ✅ CCPA：数据透明度、用户权利、数据删除
- ✅ ISO 27001：信息安全管理体系
- ✅ SOC 2：安全、可用性、处理完整性、保密性、隐私性
...
CREATE OR REPLACE FUNCTION public.detect_anomalous_access(
  ...
  p_threshold INTEGER DEFAULT 1000  -- 异常访问阈值
)
RETURNS TABLE (
  viewer_account_id UUID,
  access_count BIGINT,
...
1. **数据分类管理**：谁有权设置和修改数据分类？是否需要审批流程？
2. **合规性要求**：是否需要支持其他合规标准（如 HIPAA、PCI-DSS）？
3. **审计日志保留**：1 年的保留期是否足够？是否需要更长的保留期？
4. **异常访问处理**：检测到异常访问后，应该采取什么措施（告警、自动阻止）？
5. **数据脱敏策略**：是否需要更复杂的脱敏算法（如差分隐私）？
