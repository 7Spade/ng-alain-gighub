# 數據庫概覽

> **目的**：為 AI 提供數據庫架構的快速參考，幫助理解數據庫結構和關鍵表關係。

## 📊 數據庫架構總覽

系統共包含 **51 張資料表**，分為 11 個模組。所有表都使用 Supabase PostgreSQL，並啟用 RLS (Row Level Security) 行級安全。

## 🗂️ 模組分類

### 🔐 帳戶與身份系統 (4 張)

- **accounts** - 帳戶主表（統一身份抽象，支援 User/Bot/Organization）
- **teams** - 團隊表
- **team_members** - 團隊成員表
- **organization_schedules** - 組織排班表（跨藍圖成員調派）

### 🤝 組織協作系統 (3 張)

- **organization_collaborations** - 組織協作關係表（1:1 承攬關係）
- **collaboration_invitations** - 協作邀請表
- **collaboration_members** - 協作成員表

### 🔒 權限系統 (5 張)

- **roles** - 角色定義表
- **user_roles** - 用戶角色關聯表
- **permissions** - 權限定義表
- **role_permissions** - 角色權限關聯表
- **branch_permissions** - 分支權限表（分支層級權限控制）

### 🎯 藍圖/專案系統 (5 張) - Git-like 分支模型

- **blueprints** - 藍圖主表（主分支，擁有者組織控制任務結構）
- **blueprint_configs** - 藍圖配置表
- **blueprint_branches** - 藍圖分支表（組織分支）
- **branch_forks** - 分支 Fork 記錄表
- **pull_requests** - PR 表（分支合併請求）

### 📋 任務執行系統 (9 張)

- **tasks** - 任務主表（樹狀結構）
- **task_assignments** - 任務指派表
- **task_lists** - 待辦列表表
- **task_staging** - 暫存區表（48h 可撤回）
- **daily_reports** - 施工日誌表
- **report_photos** - 施工日誌照片表
- **weather_cache** - 天氣快取表
- **task_dependencies** - 任務依賴表
- **task_templates** - 任務模板表

### ✅ 品質驗收系統 (4 張)

- **quality_checks** - 品管檢查表
- **qc_photos** - 品管照片表
- **inspections** - 驗收表（責任切割）
- **inspection_photos** - 驗收照片表

### ⚠️ 問題追蹤系統 (4 張)

- **issues** - 問題主表
- **issue_assignments** - 問題指派表
- **issue_photos** - 問題照片表
- **issue_sync_logs** - 問題同步日誌表（跨分支同步）

### 💬 協作溝通系統 (6 張)

- **comments** - 留言表
- **notifications** - 通知表
- **notification_rules** - 通知規則表
- **notification_subscriptions** - 通知訂閱表
- **personal_todos** - 個人待辦表
- **todo_status_tracking** - 待辦狀態追蹤表

### 📊 資料分析系統 (6 張)

- **documents** - 文件主表
- **document_versions** - 文件版本表
- **document_thumbnails** - 文件縮圖表
- **progress_tracking** - 進度追蹤表
- **activity_logs** - 活動記錄表
- **analytics_cache** - 分析快取表

### 🤖 機器人系統 (3 張)

- **bots** - Bot 定義表
- **bot_tasks** - Bot 任務表
- **bot_execution_logs** - Bot 執行日誌表

### ⚙️ 系統管理 (2 張)

- **settings** - 系統設定表
- **feature_flags** - 功能開關表

## 🔗 關鍵關聯關係

### 藍圖 → 分支 → PR 流程

```
blueprints (主分支)
    ↓ 1:N
blueprint_branches (組織分支)
    ↓ 1:N
pull_requests (提交執行數據)
    ↓ 審核通過
blueprints (更新承攬欄位)
```

### 任務執行流程

```
tasks (任務樹)
    ↓
task_assignments (指派)
    ↓
task_lists (待辦列表)
    ↓
task_staging (48h 暫存)
    ↓
daily_reports (施工日誌) + quality_checks (品管)
    ↓
inspections (驗收/責任切割)
    ↓
progress_tracking (進度儀表板)
```

### 問題同步機制

```
issues (分支問題)
    ↓ 即時同步
issue_sync_logs
    ↓
blueprints (主分支統一掌控)
```

## 🔐 安全機制

- **RLS (Row Level Security)**：所有表都啟用 RLS
- **權限驗證**：資料庫層（RLS）與應用層雙重驗證
- **分支權限**：`branch_permissions` 表控制分支層級權限

## 📚 參考文檔

- [資料表清單總覽](../../docs/30-資料表清單總覽.md)
- [完整 SQL 表結構定義](../../docs/30-0-完整SQL表結構定義.md) ⭐⭐⭐⭐⭐
- [實體關係圖](../../docs/12-實體關係圖.mermaid.md)

---

**最後更新**：2025-01-15

