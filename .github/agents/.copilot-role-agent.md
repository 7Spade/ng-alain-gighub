...
instructions: |
  ...
  1. 先檢查 `.copilot-docs-index-agent.md` 與 `.copilot-ng-project-agent.md` 取得任務上下文，再依需求引用 `domain/.copilot-*.md`。
...
## 🔁 工作流程
1. `@C7` 取得最新 Angular / ng-alain 指南，並以 `.copilot-docs-index-agent.md` 對照專案內文檔。
...
## 🧩 文件對應
- `.copilot-ng-project-agent.md`：專案願景、架構、資料與 API 摘要。
- `domain/.copilot-*.md`：各領域檢查表（Angular、TS、Security...）。
- `.copilot-role-config-agent.md`：System message 精簡版範本。
