...
## 🚀 使用方式
- **Cursor / Chat 模式**：直接引用 `ng-project-agent.md` 取得專案上下文，再依任務挑選 domain agent。
- **GitHub Actions**：於 workflow 中載入 `role.agent.md` + 相關 domain 文件，詳見 `workflows/agents-workflow.md`。
- **文件導覽**：根據需求閱讀對應 templates、examples 或 meta 索引。

## 🧭 推薦閱讀順序
1. `role.agent.md` – 了解 AI 角色定位與回覆原則。
2. `ng-project-agent.md` – 獲取專案願景、架構、資料與 API 摘要。
3. `domain/*.md` – 依任務類型套用專家檢查表。
4. `templates/` & `examples/` – 建立自動化流程或 prompt。
