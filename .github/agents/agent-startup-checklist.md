...
### 🎯 第 3 步：使用 Sequential Thinking 分析任務（必須）⭐⭐⭐⭐⭐
...
// Thought 3/7: 檢查架構思維導圖
// 系統架構位置：Core Module → Authentication Layer
// 相關模組：權限控制、用戶管理、RLS 策略
// 依賴關係：需要確保 RLS 策略已配置

// Thought 4/7: 分析架構影響（遵循五層開發順序）
// 需要修改的層級：
...
// Thought 6/7: 參考最佳實踐（從記憶庫）
// - 遵循 "Four Core Development Principles"
// - 實施 "Security Best Practices"
// - 使用 "OnPush Strategy" 優化效能
// - 確保測試覆蓋率達標
...
### 📋 第 4 步：使用 Software Planning Tool 制定計畫（必須）⭐⭐⭐⭐⭐
