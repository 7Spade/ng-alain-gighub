...
4. **流程**：`@C7` 取得官方資訊 → `.copilot-docs-index-agent.md` 對應內部文件 → `@S7` 拆解 → `@SPT` 計畫 → 每個步驟皆驗證 lint/type-check/test/build。
...
## 5. Minimal Checklist
1. 引用 `.copilot-docs-index-agent.md` 找到來源，而非複製外部內容。
