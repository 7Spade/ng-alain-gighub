...
### 第三步：了解工作流程
```
1. 📖 Context7 (@C7) → 取得 Angular/ng-alain/Supabase 官方資訊
...
任務類型？
├─ 新功能開發
│  ├─ 讀取：ng-project-agent.md（了解架構）
│  ├─ 讀取：docs-index.md（找到相關資料模型）
│  └─ 使用：domain/angular-agent.md（開發規範）
│
├─ Bug 修復
│  ├─ 讀取：ng-project-agent.md（了解現有架構）
...
2. 再讀：ng-project-agent.md（了解專案架構）
3. 然後：docs-index.md（查找需要的文件）
4. 最後：選擇適當的 domain agent
```
...
### 任務：整合 Supabase API
```markdown
1. ✅ 閱讀 ng-project-agent.md（了解認證流程）
...
| `ng-project-agent.md` | 專案架構與技術棧 | 需要了解整體架構時 |
