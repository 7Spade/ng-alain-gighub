...
export class BlueprintService {
  ...
  // Computed signals
  readonly activeBlueprints = computed(() => this.blueprints().filter(b => b.status === BlueprintStatus.ACTIVE));

  readonly planningBlueprints = computed(() => this.blueprints().filter(b => b.status === BlueprintStatus.PLANNING));

  readonly completedBlueprints = computed(() => this.blueprints().filter(b => b.status === BlueprintStatus.COMPLETED));
  ...
  async loadBlueprintByProjectCode(projectCode: string): Promise<Blueprint | null> {
    ...
    try {
      const blueprint = await firstValueFrom(this.blueprintRepository.findByProjectCode(projectCode));
      if (blueprint) {
        this.selectedBlueprintState.set(blueprint);
      }
      return blueprint;
  ...
  async updateBlueprint(id: string, data: BlueprintUpdate): Promise<Blueprint> {
    ...
    try {
      ...
      // 更新本地状态
      this.blueprintsState.update(blueprints => blueprints.map(b => (b.id === id ? blueprint : b)));
  ...
  async loadConfigs(blueprintId: string): Promise<BlueprintConfig[]> {
    ...
    try {
      const configs = await firstValueFrom(this.blueprintConfigRepository.findByBlueprintId(blueprintId));
  ...
  async getConfig(blueprintId: string, configKey: string): Promise<BlueprintConfig | null> {
    return await firstValueFrom(this.blueprintConfigRepository.findByConfigKey(blueprintId, configKey));
  }
  ...
  async setConfig(blueprintId: string, configKey: string, configValue: any, updatedBy?: string): Promise<BlueprintConfig> {
    ...
    try {
      const config = await firstValueFrom(this.blueprintConfigRepository.upsertConfig(blueprintId, configKey, configValue, updatedBy));
      // 更新本地状态
