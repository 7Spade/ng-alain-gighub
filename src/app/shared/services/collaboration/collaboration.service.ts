...
export class CollaborationService {
  ...
  // Computed signals
  readonly activeCollaborations = computed(() =>
    this.collaborations().filter(c => c.status === CollaborationStatus.ACTIVE)
  );

  readonly pendingCollaborations = computed(() =>
    this.collaborations().filter(c => c.status === CollaborationStatus.PENDING)
  );
  ...
  async loadCollaborationsByBlueprintId(blueprintId: string): Promise<OrganizationCollaboration[]> {
    ...
    try {
      const collaborations = await firstValueFrom(
        this.collaborationRepository.findByBlueprintId(blueprintId)
      );
  ...
  async loadCollaborationsByOwnerOrgId(ownerOrgId: string): Promise<OrganizationCollaboration[]> {
    ...
    try {
      const collaborations = await firstValueFrom(
        this.collaborationRepository.findByOwnerOrgId(ownerOrgId)
      );
      return collaborations;
    } catch (error) {
      this.errorState.set(error instanceof Error ? error.message : '加载协作关系列表失败');
   ...
   * 根据协作组织 ID 加载协作关系
   */
  async loadCollaborationsByCollaboratorOrgId(
    collaboratorOrgId: string
  ): Promise<OrganizationCollaboration[]> {
    this.loadingState.set(true);
    this.errorState.set(null);

    try {
      const collaborations = await firstValueFrom(
        this.collaborationRepository.findByCollaboratorOrgId(collaboratorOrgId)
      );
      return collaborations;
    } catch (error) {
      this.errorState.set(error instanceof Error ? error.message : '加载协作关系列表失败');
   ...
   * 根据协作类型加载协作关系
   */
  async loadCollaborationsByType(
    ...
    try {
      const collaborations = await firstValueFrom(
        this.collaborationRepository.findByCollaborationType(collaborationType)
      );
      return collaborations;
    } catch (error) {
      this.errorState.set(error instanceof Error ? error.message : '加载协作关系列表失败');
   ...
   * 根据状态加载协作关系
   */
  async loadCollaborationsByStatus(
    ...
    try {
      const collaborations = await firstValueFrom(
        this.collaborationRepository.findByStatus(status)
      );
      return collaborations;
    } catch (error) {
      this.errorState.set(error instanceof Error ? error.message : '加载协作关系列表失败');
  ...
  async createCollaboration(
    data: OrganizationCollaborationInsert
  ): Promise<OrganizationCollaboration> {
    this.loadingState.set(true);
    this.errorState.set(null);
  ...
  async updateCollaboration(
    id: string,
    data: OrganizationCollaborationUpdate
  ): Promise<OrganizationCollaboration> {
    this.loadingState.set(true);
    this.errorState.set(null);
    ...
    try {
      ...
      // 更新本地状态
      this.collaborationsState.update(collaborations =>
        collaborations.map(c => (c.id === id ? collaboration : c))
      );
