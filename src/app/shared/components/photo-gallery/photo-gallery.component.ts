      ...
      <div class="gallery-grid">
        @for (photo of photos(); track photo.id) {
          <div class="photo-item" (click)="openLightbox(photo)">
            <img [src]="photo.thumbnailUrl || photo.url" [alt]="photo.title || '照片'" class="photo-thumbnail" />
            @if (photo.title) {
      ...
      >
        @if (currentPhoto()) {
          <div class="lightbox-content">
            <img [src]="currentPhoto()!.url" [alt]="currentPhoto()!.title || '照片'" class="lightbox-image" />
            ...
            <div class="lightbox-nav">
              <button nz-button nzType="default" [disabled]="!canGoPrevious()" (click)="previousPhoto()">
                <span nz-icon nzType="left"></span>
                上一張
              </button>
              <span class="photo-counter"> {{ currentIndex() + 1 }} / {{ photos().length }} </span>
              <button nz-button nzType="default" [disabled]="!canGoNext()" (click)="nextPhoto()">
                下一張
