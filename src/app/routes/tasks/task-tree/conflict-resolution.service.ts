...
export class ConflictResolutionService {
   ...
   */
  resolveConflict(local: VersionedTask, remote: VersionedTask, conflict: ConflictDetectionResult): ConflictResolutionResult {
  ...
  notifyConflict(notification: ConflictNotification): void {
    ...
    const exists = currentConflicts.some(
      c => c.taskId === notification.taskId && c.timestamp.getTime() === notification.timestamp.getTime()
    );
  ...
  resolveNotification(taskId: string, timestamp: Date): void {
    const updated = this.conflicts().map(c =>
      c.taskId === taskId && c.timestamp.getTime() === timestamp.getTime() ? { ...c, resolved: true } : c
    );
  ...
  private resolveLastWriteWins(local: VersionedTask, remote: VersionedTask): ConflictResolutionResult {
  ...
  private resolveMerge(local: VersionedTask, remote: VersionedTask, conflict: ConflictDetectionResult): ConflictResolutionResult {
  ...
  private resolveReject(local: VersionedTask, remote: VersionedTask): ConflictResolutionResult {
    return {
      resolved: false,
  ...
  private resolveManual(local: VersionedTask, remote: VersionedTask, conflict: ConflictDetectionResult): ConflictResolutionResult {
