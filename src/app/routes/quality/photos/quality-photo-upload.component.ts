...
@Component({
  selector: 'app-quality-photo-upload',
  standalone: true,
  ...
  template: `
    <form nz-form [formGroup]="form" (ngSubmit)="submit()">
      <nz-form-item>
        ...
        <nz-form-control [nzSpan]="18" nzErrorTip="请选择照片类型">
          <nz-select formControlName="photoType" nzPlaceHolder="选择照片类型">
            <nz-option nzValue="acceptance" nzLabel="验收照片"></nz-option>
            <nz-option nzValue="defect" nzLabel="缺陷照片"></nz-option>
            <nz-option nzValue="before_correction" nzLabel="纠正前"></nz-option>
            <nz-option nzValue="after_correction" nzLabel="纠正后"></nz-option>
            <nz-option nzValue="handover" nzLabel="移交照片"></nz-option>
          </nz-select>
        </nz-form-control>
      </nz-form-item>
      ...
      <nz-alert
        nzType="info"
        nzMessage="提示"
        nzDescription="照片上传后将关联到当前验收记录。实际项目中，照片会先上传到 Supabase Storage，然后创建 document 记录。"
        nzShowIcon
...
export class QualityPhotoUploadComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.form = this.fb.group({
      photoType: ['acceptance', [Validators.required]],
      caption: ['']
    });
  }
  ...
  async submit(): Promise<void> {
    ...
    try {
      ...
      const tempDocumentId = `temp-qc-${Date.now()}`;

      const photoRecord = {
        inspection_id: this.data.inspectionId,
