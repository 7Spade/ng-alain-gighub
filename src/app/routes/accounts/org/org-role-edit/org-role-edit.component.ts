...
export interface OrgRoleEditData {
 ...
 * - 仅支持 Modal 模式
 * - 仅负责编辑单个成员的角色
 * - 支持 OWNER、ADMIN、MEMBER 三种角色
 */
@Component({
  selector: 'app-org-role-edit',
  standalone: true,
      ...
      <nz-form-item>
        ...
        <nz-form-control [nzSpan]="18" [nzErrorTip]="'請選擇角色'">
          <nz-radio-group formControlName="role">
            <label nz-radio [nzValue]="OrganizationMemberRole.OWNER">擁有者</label>
            <label nz-radio [nzValue]="OrganizationMemberRole.ADMIN">管理員</label>
            <label nz-radio [nzValue]="OrganizationMemberRole.MEMBER">成員</label>
          </nz-radio-group>
        </nz-form-control>
      </nz-form-item>
...
export class OrgRoleEditComponent implements OnInit {
  private organizationMemberService = inject(OrganizationMemberService);
  private modalRef = inject(NzModalRef);
  private message = inject(NzMessageService);

  readonly data: OrgRoleEditData = inject(NZ_MODAL_DATA);
