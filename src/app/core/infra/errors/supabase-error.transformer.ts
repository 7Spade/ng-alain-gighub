...
export function transformSupabaseError(error: PostgrestError, source?: string): AppError {
  ...
  if (errorInfo) {
    return createAppError(errorInfo.message, errorInfo.type, errorInfo.severity, {
      code: errorCode,
  ...
  // 默认错误处理
  return createAppError(error.message || '数据库操作失败', 'unknown', 'error', {
    code: errorCode,
    details: error.details || '',
    source,
    metadata: {
 ...
 */
export function handleSupabaseResponse<T>(response: { data: T | null; error: PostgrestError | null }, source?: string): T {
  if (response.error) {
    throw transformSupabaseError(response.error, source);
  }

  if (response.data === null) {
    throw createAppError('数据不存在', 'business', 'warning', { source });
  }
